...
const UMF_VERSION: string = 'UMF/1.4.6';
...
class UMFMessage {
  ...
  * @return {undefined}
  */
  signMessage(algo: string, sharedSecret: string) {
...
* @param {IUMFHash} message - message object
* @return {undefined}
*/
function createMessageInstance(message: IUMFHash) {
  let proxy = new Proxy(new UMFMessage(), {
    get: function(target: IUMFHash, name: string, _receiver) {
 ...
 *                  isn't valid.
 */
function parseRoute(toValue: string) {
  let serviceName = '';
  let httpMethod = '';
  let apiRoute = '';
  let error = '';
